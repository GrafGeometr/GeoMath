...
class UsersFile(SqlAlchemyBase):
    ...
    def __repr__(self):
        if self.extension=='.ggb':
            return f'<iframe scr="/static/user_files/{self.id}{self.extension}"><br>'
        elif self.extension in ['.txt','.pdf','.doc','.docx']:
    ...
    def edit(self,togo):
        if self.extension=='.ggb':
            return f'<iframe scr="/static/user_files/{self.id}{self.extension}"><a href="/delete_file/{self.id}/{togo}" class="btn btn-danger">Удалить</a><br>'
        elif self.extension in ['.txt','.pdf','.doc','.docx']:
